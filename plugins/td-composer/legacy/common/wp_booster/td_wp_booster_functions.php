...
//ajax: system status - TD Log - show/hide td advertisement
add_action( 'wp_ajax_td_ajax_system_status_toggle_td_banners', array( 'td_ajax', 'on_ajax_system_status_toggle_td_banners' ) );

//ajax: get style from template
add_action( 'wp_ajax_td_ajax_get_template_style', array( 'td_ajax', 'on_ajax_get_template_style' ) );

//ajax: render page/post content
...
function load_front_js() {
	...
	if ($td_deploy_mode == 'dev') {
        ...
        wp_enqueue_script('td-site-min', TDC_URL_LEGACY . '/js/tagdiv_theme.min.js', array('jquery'), TD_THEME_VERSION, true);
    }
...
function td_bottom_code() {
    ...
    <!--
        ...
        Theme: ' . TD_THEME_NAME . ' by tagDiv.com 2023
...
function td_on_admin_body_class( $classes ) {
    ...
    // Custom service banners class
    if ( td_util::get_option('td_banners_status') === 'off' ) {
        $classes .= ' td-hide-theme-adv';
    }
